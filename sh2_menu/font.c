...
void SetFontPalette()
{
	...
	//uint8_t *pFont;
	rgb888_t Colors[4];	
	Colors[3] = RGB888(1, 0, 0, 0); //unused
...
void ClearTextOnBuffer(int left, int top, int width, int height, uint8_t * buffer)
{
	...
	for (int _y = top; ( (_y < FONT_QUAD_HEIGHT) && (_y < top+height) ) ; _y++)
	{
		p8_vram = (uint8_t *)(buffer + _y*FONT_QUAD_WIDTH);
		for (int _x = left; ( (_x < FONT_QUAD_WIDTH) && (_x < left+width) ) ; _x++)
		{
			p8_vram[_x] = 0;
		}
...
{
	ClearTextOnBuffer(0,0,FONT_QUAD_WIDTH,FONT_QUAD_HEIGHT,buffer);
}
...
void DrawCharOnBuffer(unsigned int x, unsigned int y, char c, unsigned int palette, bool transparent, uint8_t * buffer) 
{
	...
	for (int _y = 0; _y < 8; _y++)
	{
		p8_vram = (uint8_t *)(buffer + x + (_y+y)*FONT_QUAD_WIDTH);
		p8_char =  &(SuiteFont[(c-32)*32+_y*4]);
		for (int _x = 0; _x < 4; _x++)
		{
			tmp = p8_char[_x]&0x3;
...
void DrawStringOnBuffer(char *str, unsigned int x, unsigned int y, unsigned int palette, uint8_t * buffer)
{
	...
	for(i = 0; i < strlen(str); i++)
	{
		...
		DrawCharOnBuffer(x, y, str[i], palette, true, buffer);
		x += _fw;
	}
}
...
void DrawStringWithBackgroundOnBuffer(char *str, unsigned int x, unsigned int y, unsigned int palette, unsigned int bg_palette, uint8_t * buffer)
{
	...
	for (_y = 0; _y < _fh; _y++)
	{
		p8_vram = (uint8_t *)(buffer + x + (_y+y-1)*FONT_QUAD_WIDTH);
		for (_x = 0; _x < _fw*(strlen(str)+1)/2; _x++)
		{
			p8_vram[_x*2+1] = 0x1 | bg_palette*4;
			p8_vram[_x*2] = 0x1 | bg_palette*4;
