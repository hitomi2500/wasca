...
////////////////////////////////////////////////////////////////////
//
//  ALTERA_ONCHIP_FLASH_AVMM_DATA_CONTROLLER (PARALLEL)
//
...
module altera_onchip_flash_avmm_data_controller (
    ...
    reg erase_clear_busy;
    reg write_clear_busy;
	...
	generate // generate combi based on read and write mode
		if (READ_AND_WRITE_MODE == 1) begin
			...
			assign flash_addr_wire = 
				(valid_csr_erase && valid_csr_sector_erase_addr) ? { flash_sector_wire, 1'b0, {(19){1'b1}} } : 
			...
			assign flash_addr_wire = flash_page_addr_wire;
		end
	endgenerate	
	...
	assign flash_erase_addr = (valid_csr_sector_erase_addr) ? csr_sector_erase_addr : csr_page_erase_addr;
	...
	generate // generate combi based on read burst mode
		if (WRAPPING_BURST_MODE == 0) begin
			...
			assign cur_addr = (valid_csr_erase) ? flash_erase_addr : flash_read_addr;			
			...
			assign flash_se = (is_read_busy && read_state == READ_STATE_SETUP);
			...
			// wrapping read
			assign cur_addr = (valid_csr_erase) ? flash_erase_addr : avmm_addr;
			...
			assign flash_se = (read_state == READ_STATE_READ && read_ctrl_count==FLASH_READ_CYCLE_MAX_INDEX+1);
			assign avmm_waitrequest = ~reset_n || ((~is_write_busy && avmm_write) || write_wait_w || (~is_read_busy && avmm_read) || (avmm_read && read_wait_w));
		end
	endgenerate
	...
	generate // generate always block based on read and write mode. Write and erase operation is unnecessary in read only mode.
		if (READ_AND_WRITE_MODE == 1) begin
			...
			// -------------------------------------------------------------------
			always @ (negedge reset_n or posedge erase_clear_busy or posedge write_clear_busy  or posedge flash_osc) begin
				if (~reset_n || erase_clear_busy || write_clear_busy) begin
					flash_busy_reg <= 0;
					flash_busy_clear_reg <= 0;
                end
				...
				if (~reset_n_w) begin
					...
					case (write_state)
						...
						WRITE_STATE_RESET: begin
                            ...
                            if (~flash_busy_sync && ~flash_busy_clear_sync) begin
				...
				if (~reset_n_w) begin
					...
					case (erase_state)
						...
						ERASE_STATE_WAIT_DONE: begin
							if (flash_busy_clear_sync) begin
                                erase_clear_busy <= 1'b1;
								erase_count <= FLASH_RESET_CYCLE_MAX_INDEX[5:0];
								erase_state <= ERASE_STATE_RESET;
							end
							else begin
